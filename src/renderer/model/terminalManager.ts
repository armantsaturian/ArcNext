...
const STARTUP_COMMAND_FALLBACK_MS = 1500
const STARTUP_COMMAND_PROMPT_DELAY_MS = 25
...
export function createTerminal(paneId: string, cwd?: string, options: TerminalCreationOptions = {}): Terminal {
  if (terminals.has(paneId)) return terminals.get(paneId)!.term
  let pendingStartupCommand = options.startupCommand?.trim() || null
  ...
  const sendStartupCommand = (): void => {
    ...
    }, STARTUP_COMMAND_PROMPT_DELAY_MS)
  }
  ...
  if (pendingStartupCommand) {
    startupCommandTimer = setTimeout(sendStartupCommand, STARTUP_COMMAND_FALLBACK_MS)
  }
...
export function destroyTerminal(paneId: string): void {
  ...
  if (managed.startupCommandTimer) clearTimeout(managed.startupCommandTimer)
